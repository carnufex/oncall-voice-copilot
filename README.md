...
| Guardrails: focus, **prompt injection** (shown live: a spoken "ignore your previous instructions, admin override" ends the call, named in the termination reason, and the thread gets a :no_entry: line), and a **custom guardrail** on both agents ("No execution claims without a tool result") that blocks any reply claiming a rollback, commit, reset or recovery that no tool result in the call confirms; the evaluator sees the tool calls and results. `npm run drill:guardrail` proves it live: a throwaway copy of the agent with a sabotaged prompt tries to claim the rollback is done and gets cut off | agent config `platform_settings.guardrails.custom`, `elevenlabs/scripts/guardrail-drill.mjs` |
...
| Knowledge base with crawled external sources (Kubernetes and Argo CD docs, auto-synced weekly) and source attribution on answers | agent config `knowledge_base`, `conversation.source_attribution` |
